...
<html>
<head>
	<title>Classroom Checking Form</title>
...
-->
	<div id="content" style="width:100%; margin-right:auto; margin-left:auto;">
		...
		<?php include('../../checklist/includes/menu.inc.php'); include ('../../checklist/includes/checks_logo.php'); ?>
		...
		<div id='click_to_begin' style='position:relative; top:-85px;'>
			...
			<?php
				...
				//	} else {
					...
					print "<button name='confirm_report' id='confirm_report' value='SUBMIT' onClick='nothing()'>SUBMIT</button> &nbsp; <button name='redo_report' id='redo_report' value='BACK' onClick='history.go(-1)'>back</button>";		// onClick changed from 'submitDat()' in order to disable it for now!!!!!!!!	
		...
		<div id="buttons" style="display:none; padding:10px; position:relative; top:-85px;">
				<button name="real_call" id="real_call" onClick="document.getElementById('form_real_calls').style.display=''; document.getElementById('form_other_calls').style.display='none'; document.boxes.support.value='yes';">Classroom Checking Form</button>
				 &nbsp; &nbsp; 
		...
		<div id='form_real_calls' style='display:; padding:10px; position:relative; top:-85px;'>
			<?php 
		...
		<div id="equipment_checkboxes" style="display:none; padding:10px; text-align:left; position:relative; top:-85px; margin-left:auto; margin-right:auto; width:400px;">
			...
			<select name="lab" id="lab">
				<option value="st">ST Lab</option>
				<option value="pac">PAC Lab</option>
				<option value="has">HAS Lab</option>
				<option value="keck">KECK Lab</option>
				<option value="cfa">CFA Lab</option>
				<option value="olin">Olin Lab</option>
			</select>
			<p>
			Were there any problems when you first checked the room?<br />
			<input type="radio" name="radio_resolved" value="yes" onClick="document.getElementById('fixed').style.display=''; document.getElementById('type').style.display='';" /> Yes <input type="radio" name="radio_resolved" value="no" checked onClick="document.getElementById('fixed').style.display='none'; document.getElementById('type').style.display='none';" /> No
		
			</p>
			<p id="type" style="display:none;" >General type of problem:<br />
			<input type="checkbox" name="check_ue" value="yes" /> User Error <input type="checkbox" name="check_ef" value="no" /> Equipment Failure <br /><input type="checkbox" name="check_em" value="missing" /> Equipment Missing <input type="checkbox" name="check_si" value="software"> Software Issue	
			</p>
	...
	-->
			<p id="fixed" style="display:none;">
			Were you able to fix the problems?<br />
			<input type="radio" name="radio_tech_solved" value="yes" onClick="document.getElementById('rt').style.display='none'" /> Yes <input type="radio" name="radio_tech_solved" value="no" onClick="document.getElementById('rt').style.display=''" /> No <input type="radio" name="radio_tech_solved" value="unsure" onClick="document.getElementById('rt').style.display=''" /> Some, but not all
			</p>
			<p id="rt" style="display:none;">
			Creating/Adding to a ticket in <a href="http://rt.wesleyan.edu" target="_blank">RT</a>?<br />
			<input type="radio" name="radio_rt" value="yes" onClick="document.getElementById('ticket').style.display=''" /> Yes <input type="radio" name="radio_rt" value="no" onClick="document.getElementById('ticket').style.display='none'" /> No<br />
		...
		-->
			<div id="bottom_submit_button_div" style="position: relative; left:150px; padding:10px;">
				<input type="button" id="bottom_submit_button" value="SUBMIT REPORT" onClick="document.boxes.submit();" />
			</div>
		</div>
	</div>
