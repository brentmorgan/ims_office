...
function listCalls(){
	...
	$list .= "<table border='1' width='100%' style='text-align:center;'><tr><th>Edit</td><th>Date</th><th>Reported By</th><th>Room</th><th>Resolved Over Phone?</th><th>Type of Problem</th><th>Sent a Tech?</th><th>Tech Solved Problem?</th><th>RT Ticket #</th></tr>";
	...
	foreach($another_call_list as $row){ 
		...
		$list .= "<tr id='row_num_" . $row['id'] . "'><td id='edit_" . $row['id'] . "'><a href='editcall.php?id=" . $row['id'] . "'>Edit</a></td><td>" . $row['datecreated'] . "</td><td>" . $row['reported_by'] . "</td><td> " . $row['room_id'] . "</td><td>" . $row['resolved_phone'] . "</td><td id='problem_" . $row['id'] . "'>" . $problem . "</td><td>" . $row['send_tech'] . "</td><td id='solved_" . $row['id'] . "'>" . $row['solved_tech'] . "</td><td id='rt_" . $row['id'] . "'>" . $rt . "</td></tr>";
	}
	$list .= "</table>";
