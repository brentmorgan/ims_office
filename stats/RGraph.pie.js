    ...
    */
    RGraph.Pie = function (id, data)
    {
        ...
        */
        ...
        this.properties['chart.colors']         = ['rgb(200,200,20)',   '#ddd',
        ...
        this.properties['chart.labels.sticks']   = false;
        ...
        */
        if (this.Get('chart.tooltips').length) {
            ...
            */
			...
			//	alert(obj.Get('chart.tooltips')[i]);
			// THIS PART IS BEING ADDED BY BRENT
			// WHEN A PIE SLICE IS CLICKED ON, IF IT WAS A 'BUILDING' PIE SLICE FROM THE CALLS BY BUILDING PIE WE WANT TO GET MORE INFO ABOUT THAT BUILDING
			//	checkForBuildingPie(obj.Get('chart.tooltips')[i]);
			//  I WILL NOW MAKE A CHECKFORBUILDINGPIE() FUNCTION ON ANOTHER .JS PAGE
			// this needs to be called in cases other than when it is a building  - we need to draw pies for other reasons too
			// so I'm changin the nemae of the function to make it more understandable
				shouldWeDrawAnotherPie(obj.Get('chart.tooltips')[i],this.id);                        
				//alert(this.id);
			}
    ...
    */
    RGraph.Pie.prototype.DrawSegment = function (degrees, color, last)
    {
        ...
        /**
        * Draw the stick for labels if required
        */
        if (this.Get('chart.labels.sticks')) {
            ...
            var midpoint = (subTotal + (degrees / 2)) / 57.3;
