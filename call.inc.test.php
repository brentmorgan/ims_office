...
function getClassroomCalls($thisDay){
	...
	$sql = "SELECT *, report_details.id as the_id from report_details JOIN report_building ON report_details.report_id=report_building.id WHERE DATE_FORMAT(report_details.datecreated, '%Y%m%d') = '".$thisDay."' AND phone_code != '0'; ";
...
function listCalls(){
	...
	foreach($another_call_list as $row){
		$edit_me = $row['status']." <br /><form name=\'edit_info_form_".$row['the_id']."\' style=\'margin:10px;\'><textarea name=\'new_text\' cols=\'60\' rows=\'3\'>Not yet functional</textarea><input type=\'submit\' value=\'update\' style=\'margin:10px;\' /></form>";
		$list .= "<tr><td>" . $row['datecreated'] . "</td><td>" . $row['reportedby'] . "</td><td> " . $row['room'] . "</td><td id='notes_field_".$row['the_id']."'>" . $row['status'] . " <a href='#' style=\"float:right;\" onClick=\"document.getElementById('notes_field_".$row['the_id']."').innerHTML='".$edit_me."'; \">follow up</a></td></tr>";
	}
	$list .= "</table>";
