...
//	$list .= "<form name='RoomsForm' method='post' action=''>";
	$list .= "<select id='room' name='room' onChange=\"document.getElementById('equipment_checkboxes').style.display=''; log_roomid=(this.value);\">"; //THIS SHOULD BE OPENDIV SO ITS ALL FANCY BUT I CANT MAKE IT WORK YET ??? ***** I am removing this text from the onChange "getOldIssues(this.value);"
...
// *OLD*	$sql = "SELECT * from calls WHERE DATE_FORMAT(datecreated, '%Y%m%d') = '".$thisDay."' ORDER BY datecreated DESC";
	$sql = "SELECT calls.id, reported_by, datecreated, building_id, room_id, resolved_phone, user_error, send_tech, solved_tech, confirm, rt_ticket, ticket_num, day_of_week, date, week, month, year, call_id, problem FROM calls LEFT JOIN problems ON calls.id = problems.call_id WHERE DATE_FORMAT(datecreated, '%Y%m%d') = '".$thisDay."' ORDER BY datecreated DESC";
...
function listCalls(){
	...
	$list .= "<table border='1' width='100%' style='text-align:center;'><tr><th>Edit</td><th>Date</th><th>Reported By</th><th>Room</th><th>Resolved Over Phone?</th><th>Reported Type of Problem</th><th>Sent a Tech?</th><th>Tech Solved Problem?</th><th>Confirm Type of Problem</th><th>Equipment</th><th>RT Ticket #</th></tr>";

	$counter = 0; // This is just a counter so we can have every other row shaded
	...
	foreach($another_call_list as $row){ 
		...
		switch ($row['confirm']) {
			...
			case "unsure":
				$confirm = "unsure";
				break;
			case "yes": // Yes it is user error
				$confirm = "User Error";
				break;
			case "no": // No it wasn't user error
				$confirm = "Equipment Failure";
				break;
			case "software":
				$confirm = "Software Issue";
				break;
		}
		if ($confirm == $problem) {
			$confirm = "Same as Reported";
		}

		// ******** end CONFIRM type of problem
		...
		// ************** Acounting for NULL values when no Equipment was listed
		if (strlen($row['problem']) < 1) {
			$row['problem'] = "n/a";
		}

		// ************** This is to set the background of the row either white or gray
		if (($counter%2)==1) {
			$bg = "shaded";
		} else {
			$bg = "";
     ...
     // ********************* THE URL TO EDIT AN ENTRY      has been      CHANGED!!!!! (LOOSE THE EDIT. AT THE START!!!!!!
		$list .= "<tr id='row_num_" . $row['id'] . "' ><td id='edit_" . $row['id'] . "'><a href='editcall.php?id=" . $row['id'] . "'>Edit</a></td><td>" . $row['datecreated'] . "</td><td>" . $row['reported_by'] . "</td><td> " . $row['room_id'] . "</td><td>" . $row['resolved_phone'] . "</td><td id='problem_" . $row['id'] . "'>" . $problem . "</td><td>" . $row['send_tech'] . "</td><td id='solved_" . $row['id'] . "'>" . $row['solved_tech'] . "</td><td>" . $confirm . "</td><td>" . $row['problem'] . "</td><td id='rt_" . $row['id'] . "'>" . $rt . "</td></tr>";
	}
	$list .= "</table>";
